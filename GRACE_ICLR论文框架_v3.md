...
\(A\le0\)（预测器不优于题级均值）时不盈利。带 \(p\le1\) 与 \(p_{\min}\) 截断时闭式变为上界，附录给出。同题多轨迹分块时用整块梯度协方差，不把方差机械除以轨迹数。所有量都能在 §4 审计里测得，Fig. 6c 验证。
...
A.7 Theorem 1：代入 Neyman 解，\(\mathbb E[pc]=S/\sqrt\lambda\)，\(\mathbb E[(1/p-1)r]=\sqrt\lambda S-\mathbb E r\)；令 \(s=\sqrt\lambda\)，\(f(s)=(c_0+S/s)(A+sS)\)；\(A>0\) 时 \(s^*=\sqrt{A/c_0}\)，\(f(s^*)=(\sqrt{c_0A}+S)^2\)；\(A\le0\) 时 \(f\) 单调，最优在 \(p\equiv1\)。再给 \(p\le1\) 与 \(p_{\min}\) 截断下的上界，以及同题分块协方差的修正。
