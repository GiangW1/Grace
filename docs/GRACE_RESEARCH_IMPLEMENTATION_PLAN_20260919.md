...
同日后续：用户已授权并实现独立的完全离线预测器及多卡 rollout 对照，见 [实现记录](OFFLINE_PARALLEL_CONTROL_20260919.md)。下文 §9.1 保留首批在线固定 U 的实现边界；顺序7现有单 actor＋多 rollout worker 路径，真实 GPU 验证待运行。顺序5和6仍未实现。
